-- Schedule the isolated project-reminders Edge Function every 15 minutes.
-- Reuses the existing public gateway JWT stored in Vault; service-role secrets
-- remain server-side and are never embedded in migrations.
...
begin
  ...
  select jobid into existing_job
    from cron.job
   where jobname = 'project-reminders-every-15-minutes';
...
select cron.schedule(
  'project-reminders-every-15-minutes',
  '*/15 * * * *',
  $cron$
  select net.http_post(
    url := 'https://pskbdeqaajprfhrjortm.supabase.co/functions/v1/project-reminders',
